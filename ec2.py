...
def get_image_id (region_name):
    ...
    filters = [
        {'Name': 'owner-id', 'Values': ['344085891942']}, 
        {'Name':'description', 'Values':['Amazon Linux 2 Kernel 5.10 AMI 2.0.20221210.1 x86_64 HVM gp2']}
    ]
...
def create_security_group(vpc_id, region_name):
    ...
    sec_group = ec2.create_security_group(
        GroupName='project_group', Description='project_group', VpcId=vpc_id)
...
def get_security_group_id(region_name):
    ...
    response=client.describe_security_groups(
        Filters=[
            {'Name': 'group-name', 'Values': ['project_group']}
        ],
    )
